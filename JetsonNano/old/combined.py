...
# Updated GStreamer pipeline string.
# This pipeline takes BGR frames from appsrc, converts them to NVMM memory with I420 format,
# encodes them with NVIDIA’s hardware encoder, and streams via UDP.
gst_str = (
    "appsrc ! "
    "videoconvert ! "
    "nvvidconv ! "
    "video/x-raw(memory:NVMM),format=I420,width={width},height={height},framerate={fps}/1 ! "
    "nvv4l2h264enc bitrate=2000000 ! "
    "h264parse ! "
    "mpegtsmux ! "   # Multiplex the stream into an MPEG-TS container
    "udpsink host=192.168.1.100 port=8554"
).format(width=CAM_WIDTH, height=CAM_HEIGHT, fps=TARGET_FPS)
...
def main():
    # Initialize the GStreamer video writer (using the defined pipeline)
    video_writer = cv2.VideoWriter(gst_str, cv2.CAP_GSTREAMER, 0, TARGET_FPS, (CAM_WIDTH, CAM_HEIGHT))
    if not video_writer.isOpened():
        print("Error: Could not open GStreamer video writer.")
        return
