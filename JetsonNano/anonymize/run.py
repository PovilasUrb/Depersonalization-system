...
# Convert environment variables from strings into the right data types
SHOW_FPS = os.getenv("SHOW_FPS", "False").lower() == "true"

# ----------------------
# FACE DETECTION SETTINGS
# ----------------------
FACE_DETECTOR_ENABLED = os.getenv("FACE_DETECTOR_ENABLED", "True").lower() == "true"
FACE_MODEL_FILE = os.getenv("FACE_MODEL_FILE", "res10_300x300_ssd_iter_140000.caffemodel")
FACE_CONFIG_FILE = os.getenv("FACE_CONFIG_FILE", "deploy.prototxt")
FACE_CONFIDENCE_THRESHOLD = float(os.getenv("FACE_CONFIDENCE_THRESHOLD", "0.5"))

KALMAN_ENABLED = os.getenv("KALMAN_ENABLED", "True").lower() == "true"
IOU_THRESHOLD = float(os.getenv("IOU_THRESHOLD", "0.3"))
MAX_MISSED_FRAMES = int(os.getenv("MAX_MISSED_FRAMES", "5"))
RED_BOX_SCALE = float(os.getenv("RED_BOX_SCALE", "1.4"))

# ----------------------
# POSE ESTIMATION SETTINGS
# ----------------------
POSE_ESTIMATOR_ENABLED = os.getenv("POSE_ESTIMATOR_ENABLED", "True").lower() == "true"
POSE_CHECKPOINT_PATH = os.getenv("POSE_CHECKPOINT_PATH", "checkpoint_iter_370000.pth")
POSE_HEIGHT_SIZE = int(os.getenv("POSE_HEIGHT_SIZE", "160"))
POSE_UPSAMPLE_RATIO = int(os.getenv("POSE_UPSAMPLE_RATIO", "1"))
POSE_FP16 = os.getenv("POSE_FP16", "False").lower() == "true"
POSE_CONFIDENCE_THRESHOLD = float(os.getenv("POSE_CONFIDENCE_THRESHOLD", "0.6"))
TRACK_POSE = os.getenv("TRACK_POSE", "True").lower() == "true"
SMOOTH_POSE = os.getenv("SMOOTH_POSE", "True").lower() == "true"
POSE_FACE_ANONIMYZE = os.getenv("POSE_FACE_ANONIMYZE", "True").lower() == "true"
    ...
    try:
        while True:
            ...
            if FACE_DETECTOR_ENABLED:
                ...
                for trk in trackers:
                    ...
                    if trk.time_since_update > 0 and KALMAN_ENABLED:
                        ...
                        ex = int(cx + exp_w / 2)
                        ey = int(cy + exp_h / 2)
                        fill_color = (0, 0, 0)  # "red" box replaced with black for uncertain
                    else:
            ...
            if POSE_ESTIMATOR_ENABLED:
                ...
                if TRACK_POSE:
                    # We pass an empty list for "previous_poses" here for simplicity;
                    # adapt if you want continuous multi-frame pose tracking
                    track_poses([], new_poses, threshold=3, smooth=SMOOTH_POSE)

                # Draw poses & anonymize
                for p in new_poses:
                    p.draw(frame)
            ...
            latency = processing_done_time - t0

            # Calculate instantaneous FPS (1 / latency).
            # If latency is very small, to avoid division by zero:
            fps_now = 1.0 / latency if latency > 0 else 0.0

            # Gather face and skeleton counts
            face_count = len(trackers) if FACE_DETECTOR_ENABLED else 0
            skeleton_count = len(new_poses) if POSE_ESTIMATOR_ENABLED else 0
            ...
            try:
                resp = requests.post("http://127.0.0.1:80/metrics", json=data_to_send)
